    ...
    <template v-else>
      ...
      <section class="report-panel">
        ...
        <div class="report-overview-grid">
          ...
          <article class="report-card"><div class="report-card-title">销量前10商品分析</div><table class="report-metric-table"><tbody><tr v-for="r in topRows" :key="r.label"><th>{{ r.label }}</th><td>{{ r.value }}</td></tr></tbody></table></article>
          <article class="report-card"><div class="report-card-title">新品分析</div><table class="report-metric-table"><tbody><tr v-for="r in newRows" :key="r.label"><th>{{ r.label }}</th><td>{{ r.value }}</td></tr></tbody></table></article>
        </div>
      </section>
      ...
      <div v-if="priceBucketModal.open" class="report-modal-mask" @click.self="closePriceBucketModal">
        <div class="report-modal-card">
          ...
          <div class="report-modal-body">
            <table class="report-table report-modal-table">
              <thead>
                <tr>
                  <th>标题</th>
                  <th>ASIN</th>
              ...
              <tbody>
                <tr v-for="row in priceBucketModal.rows" :key="`${row.asin}-${row.title}`">
                  <td class="report-modal-title-cell">{{ row.title || "-" }}</td>
                  <td>{{ row.asin || "-" }}</td>
...
type RankRow = { name: string; count: number; units: number; amount: number; newUnits: number; share: number };
type Dist = { label: string; count: number; units: number; share: number; extra?: number; name?: string };
...
const isNew = (row: Row) => (n(row.availabledays) ?? 999999) <= 180;
...
const fc = (v: number | null) => v === null || Number.isNaN(v) ? "-" : `$${v.toLocaleString("en-US", { maximumFractionDigits: 2 })}`;
...
const group = (rows: Row[], field: string) => { const total = sum(rows.map(r => n(r.totalunits))); const m = new Map<string, RankRow>(); rows.forEach(row => { const key = t(row[field]) || "未知"; const cur = m.get(key) || { name:key,count:0,units:0,amount:0,newUnits:0,share:0 }; const units = n(row.totalunits) || 0; cur.count += 1; cur.units += units; cur.amount += n(row.totalamount) || 0; if (isNew(row)) cur.newUnits += units; m.set(key, cur); }); return [...m.values()].sort((a,b)=>b.units-a.units).map(r => ({ ...r, share: total ? r.units/total : 0 })); };
...
const topProducts = computed(() => [...snapshotRows.value].sort((a,b)=>(n(b.totalunits)||0)-(n(a.totalunits)||0)).slice(0,10));
...
const topRows = computed<Metric[]>(() => [{ label:"销量前10商品样本总数", value:fi(topProducts.value.length) }, { label:"销量前10商品BSR均值", value:fi(avg(topProducts.value.map(r=>n(r.bsrrank)))) }, { label:"销量前10商品近30天均销量", value:fi(sum(topProducts.value.map(r=>n(r.totalunits)))) }, { label:"销量前10商品近30天均销售额", value:fc(sum(topProducts.value.map(r=>n(r.totalamount)))) }, { label:"销量前10商品平均价格", value:fc(avg(topProducts.value.map(r=>n(r.price)))) }, { label:"销量前10商品近30天评论平均增长数", value:fd(avg(topProducts.value.map(r=>n(r.reviewsdelta))),1) }, { label:"销量前10商品平均评论数", value:fd(avg(topProducts.value.map(r=>n(r.reviews))),0) }, { label:"销量前10商品平均星级", value:fd(avg(topProducts.value.map(r=>n(r.rating))),1) }]);
const newRows = computed<Metric[]>(() => [{ label:"新品数量", value:fi(metric.value.newCount) }, { label:"新品占比", value:fp(metric.value.newRatio) }, { label:"新品评分数(最高/平均/最低)", value:`${fd(metric.value.newMax,1)}/${fd(metric.value.newAvgRating,1)}/${fd(metric.value.newMin,1)}` }, { label:"新品平均价格", value:fc(metric.value.newAvgPrice) }, { label:"新品近30天均销量", value:fi(metric.value.newUnits) }, { label:"新品近30天均销售额", value:fc(metric.value.newAmount) }, { label:"商品首次上架时间", value:fdate(metric.value.firstDate) }, { label:"商品最新上架时间", value:fdate(metric.value.lastDate) }]);
...
const lineBar = (points: Dist[], bar: string, line: string, second?: string, rankLabel = false): EChartsOption => ({
  grid:{ left:50,right:48,top:28,bottom:58,containLabel:true },
  tooltip:{
    trigger:"axis",
    formatter: (params) => {
      ...
      const lines = [`<div>${point?.name || point?.label || "-"}</div>`];
      seriesList.forEach((item) => {
        lines.push(`<div>${item.marker}${item.seriesName}: ${item.value}${item.seriesName === line ? "%" : ""}</div>`);
      });
      return lines.join("");
    },
  },
  ...
  series:[
    { name:bar, type:"bar", barMaxWidth:24, data:points.map(i=>i.units || i.count), itemStyle:{ color:"#ffad2f" } },
    ...(second ? [{ name:second, type:"bar", barMaxWidth:24, data:points.map(i=>i.extra || 0), itemStyle:{ color:"#a5b4fc" } }] : []),
...
const productOption = computed(() => lineBar(concentration("product"), "销量", "销量占比", "新品销量", true)), brandOption = computed(() => lineBar(concentration("brand"), "销量", "销量占比", "新品销量", true)), sellerOption = computed(() => lineBar(concentration("seller"), "销量", "销量占比", "新品销量", true));
const salesTrendOption = computed<EChartsOption>(() => ({ grid:{ left:50,right:56,top:28,bottom:44,containLabel:true }, tooltip:{ trigger:"axis" }, legend:{ top:0, data:["销量","销售额","平均BSR"] }, xAxis:{ type:"category", data:historyByMonth.value.map(i=>i.label) }, yAxis:[{ type:"value", name:"销量" }, { type:"value", name:"销售额($)" }, { type:"value", name:"平均BSR", show:false }], series:[{ name:"销量", type:"bar", data:historyByMonth.value.map(i=>i.units), itemStyle:{ color:"#ffad2f" }, barMaxWidth:28 }, { name:"销售额", type:"line", yAxisIndex:1, smooth:true, data:historyByMonth.value.map(i=>i.amount), itemStyle:{ color:"#62c554" }, lineStyle:{ color:"#62c554", width:2 } }, { name:"平均BSR", type:"line", yAxisIndex:2, smooth:true, data:historyByMonth.value.map(i=>i.avgBsr), itemStyle:{ color:"#c7cbd4" }, lineStyle:{ color:"#c7cbd4", width:2 } }] }));
...
const sellerTypeQualityOption = computed<EChartsOption>(() => {
  ...
  return {
    grid:{ left:50,right:54,top:24,bottom:40,containLabel:true },
    tooltip:{
      trigger:"axis",
      confine: true,
    ...
    yAxis:[{ type:"value", name:"评分数" }, { type:"value", name:"评分值", max:5 }],
    series:[
...
const sellerNationOption = computed(() => lineBar(byField("sellernation", 10), "产品数量", "销量占比"));
const listingAgeOption = computed(() => lineBar(bucket(snapshotRows.value, r => n(r.availabledays), [0,30,90,180,365,730,1095,999999], ["1个月内","1-3个月","3-6个月","6-12个月","1-2年","2-3年","3年以上"]), "产品数量", "销量占比"));
const listingTrendOption = computed(() => { const total = metric.value.units || 0, map = new Map<string, Dist>(); snapshotRows.value.forEach(r => { const label = dateMs(r) ? String(new Date(dateMs(r)!).getFullYear()) : "未知", cur = map.get(label) || { label, count:0, units:0, share:0 }; cur.count += 1; cur.units += n(r.totalunits) || 0; map.set(label, cur); }); return lineBar([...map.values()].sort((a,b)=>a.label.localeCompare(b.label)).map(i => ({ ...i, share: total ? i.units/total : 0 })), "产品数量", "销量占比"); });
const reviewOption = computed(() => lineBar(bucket(snapshotRows.value, r => n(r.reviews), [0,1,50,100,200,300,500,999999], ["无评论数","1-50","50-100","100-200","200-300","300-500","500以上"]), "产品数量", "销量占比"));
const ratingOption = computed(() => lineBar(bucket(snapshotRows.value, r => n(r.rating), [0,2,3,3.5,4,4.3,4.5,5.1], ["2.0以下","2.0-3.0","3.0-3.5","3.5-4.0","4.0-4.3","4.3-4.5","4.5以上"]), "产品数量", "销量占比"));
